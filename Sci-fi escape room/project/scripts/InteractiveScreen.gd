...
export(float) var size_x = 1.0
export(float) var size_y = 1.0
...
onready var screen_cursor = $Quad/Cursor
...
func _ready():
	...
	# Generate unique mesh at defined size
	var new_quad_mesh = node_quad.mesh.duplicate()
	new_quad_mesh.size.x = size_x
	new_quad_mesh.size.y = size_y
	node_quad.set_mesh(new_quad_mesh)
	
	# Generate unique collision shape for new mesh
	var half_x = size_x / 2
	var half_y = size_y / 2
	var new_collision_shape = node_collision.shape.duplicate()
	new_collision_shape.set_points([
			Vector3(half_x, half_y, 0.1),
			Vector3(-half_x, -half_y, 0.1),
			Vector3(half_x, -half_y, 0.1),
			Vector3(-half_x, half_y, 0.1)
		])
	node_collision.set_shape(new_collision_shape)
	
	# Move content child under viewport (Fixes bug with editing children of the scene)
	if content:
		remove_child(content)
		node_viewport.add_child(content)
		
		# Set size of viewport from content child
		node_viewport.size.x = content.rect_size.x
		node_viewport.size.y = content.rect_size.y
...
func de_select():
	...
	screen_cursor.hide()
	if content and content.has_method("de_select"):
		content.de_select()
	return true
...
func handle_mouse(event):
	...
	# Move on-screen cursor to mouse position
	screen_cursor.translation = mouse_pos3D
	if not screen_cursor.visible:
		screen_cursor.show()
